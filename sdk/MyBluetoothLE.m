...
int writeType = CBCharacteristicWriteWithResponse; //CBCharacteristicWriteWithoutResponse or CBCharacteristicWriteWithResponse
...
- (void)imSendMessage:(NSString *)message{
    [Function printLog:[NSString stringWithFormat:@"imSendMessage connectStatus= %d",connectStatus]];
...
//                [self cancelCommandTimer];
//                //Command Timeout 5s
//                commandTimer = [NSTimer scheduledTimerWithTimeInterval:10.0f target:self selector:@selector(commandTimeout) userInfo:nil repeats:NO];
//            }
        }
            break;
    }
    ...
    if (error) {
        ...
        if ([peripheralDictionary containsObject:peripheral]){
            ...
            for (CBCharacteristic *characteristic in service.characteristics){
                ...
                [Function printLog:[NSString stringWithFormat:@"Service UUID : %@", characteristic.service.UUID]];
                [Function printLog:[NSString stringWithFormat:@"Characteristic UUID : %@", characteristic.UUID]];
...
- (void) commandTimeout{
    if(commandStatus == false){
        [Function printLog:@"Command Timeout 10s End"];
        [self imDisconnect];
    }
}
